...
`nbidal18-temperature-2.0.0+1.21.1.jar` is a source-retained nbidal18 component, and is the mod shipped through v4.2.8 as `nbidal18-held-heat` renamed and widened. It carries three rules. A carried torch or lantern warms you: Frostiful warms an entity from the block light at its feet, so a placed lantern warms whoever stands beside it while a carried one did nothing, because the pack's dynamic lighting is client-side and never changes the light the server reads. Insulation is symmetric: whatever an armour piece is worth against the cold it costs against the heat, applied through Thermoo's `FROST_RESISTANCE` and `HEAT_RESISTANCE` item attributes, calibrated so that a full fur set is the 1.0 that means complete protection. And gold and netherite resist heat only while their wearer is in the Nether, which no datapack can express because neither Scorchful's tags nor Thermoo's attributes know the dimension. Its MIT license is retained at `licenses/Temperature-MIT.txt`, and complete source is retained under `5. modpack source/custom mods/nbidal18-temperature` in the organized release folder.
