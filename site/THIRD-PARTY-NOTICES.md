...
`nbidal18-wiki-1.4.0+1.21.1.jar` is a source-retained nbidal18 client component. It is the pack's own in-game wiki: a keybind, or a button on the pause menu, opens a book of sections explaining how this modpack works, with no item to carry and nothing gated. It draws Field Guide's book, ruled-page, tab and page-turn textures, referenced by resource location and loaded from that mod's own JAR at runtime. **No third-party asset is copied into or redistributed with this component**; it declares Field Guide as a hard dependency because without it there would be nothing to draw, and a resource pack that restyles Field Guide restyles the wiki with it. Note that Field Guide's assets are All Rights Reserved, and its book art is itself adapted from another mod under a permission granted to that project, so the permission covering this reuse is the pack owner's to hold rather than anything this notice conveys. Earlier versions rebuilt the look from vanilla's book and advancement sprites instead; that was replaced on the owner's call because it did not match. Its MIT license is retained at `licenses/Wiki-MIT.txt`, and complete source, including every line of the wiki's text, is retained under `5. modpack source/custom mods/nbidal18-wiki` in the organized release folder.
