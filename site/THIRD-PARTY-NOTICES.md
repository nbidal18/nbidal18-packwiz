...
This Packwiz release distributes the reviewed third-party client files through its managed update channel. Project links and retained attributions for notable components follow. The release version is not repeated here on purpose: it lives in `PACK-VERSION.txt` alone, and a copy in this file would be one more place to forget.
...
`nbidal18-playeremotes-fa-bridge-1.3.0+1.21.1.jar` is a source-retained nbidal18 client compatibility component. It coordinates Player Emotes and Carry On with EMF/Fresh Animations so emotes and Carry On's raised-arm pose are not overwritten, and it pauses custom animation only for reconstructed carried-entity UUIDs to prevent their jitter. Its MIT license is retained at `licenses/PlayerEmotes-FA-CarryOn-Bridge-MIT.txt`, and its complete source is retained under `5. modpack source/custom mods/nbidal18-playeremotes-fa-bridge` in the organized release folder.

`nbidal18-grave-currency-1.2.0+1.21.1.jar` is a source-retained nbidal18 compatibility component. It stores a player's Numismatic Overhaul purse in their gravestone on death and returns it when the grave is collected, so banked money and carried coins behave the same way; without it the purse is the only thing death leaves untouched. It also makes gravestones breakable, which Gravestones itself builds as unbreakable with no setting to change it, and destroys a broken grave's contents rather than dropping them. Explosions destroy a grave too, which needs more than lowering its blast resistance: Gravestones overrides `onExplosionHit` with an empty body, and in vanilla that method is what removes an exploded block. Its MIT license is retained at `licenses/Grave-Currency-MIT.txt`, and complete source is retained under `5. modpack source/custom mods/nbidal18-grave-currency` in the organized release folder.

`nbidal18-temperature-2.1.0+1.21.1.jar` is a source-retained nbidal18 component, and is the mod shipped through v4.2.8 as `nbidal18-held-heat` renamed and widened. It carries five rules. A carried torch or lantern warms you: Frostiful warms an entity from the block light at its feet, so a placed lantern warms whoever stands beside it while a carried one did nothing, because the pack's dynamic lighting is client-side and never changes the light the server reads. Insulation is symmetric: whatever an armour piece is worth against the cold it costs against the heat, applied through Thermoo's `FROST_RESISTANCE` and `HEAT_RESISTANCE` item attributes, calibrated so that a full fur set is the 1.0 that means complete protection. A fire in the blocks immediately around you heats you, which Frostiful will not do because its whole passive contribution stops the moment you are above neutral, so before this the only flame that could overheat you was one held in the hand. A hat shades you: Scorchful hardcodes that protection onto its own sun hat and its tag drives nothing but a HUD overlay, so every hat this pack added to that tag was purely decorative until now - and because a trinket contributes no vanilla attribute modifiers, the protection is applied by scanning what is worn rather than through the item. And gold and netherite resist heat only while their wearer is in the Nether, which no datapack can express because neither Scorchful's tags nor Thermoo's attributes know the dimension. Its MIT license is retained at `licenses/Temperature-MIT.txt`, and complete source is retained under `5. modpack source/custom mods/nbidal18-temperature` in the organized release folder.

`nbidal18-jobs-reset-1.0.0+1.21.1.jar` is a source-retained nbidal18 component. It zeroes every JobsAddon job level when a player dies, so death costs job rank the same way LevelZ's `levelRetainPercentage` makes it cost skill levels; JobsAddon's own `resetCurrentJobXP` clears only the experience earned toward the current level. It uses JobsAddon's public API through a Fabric respawn listener and adds no mixin. It is licensed **GPL-3.0-only** rather than MIT, matching JobsAddon, whose classes it compiles against; its license is retained at `licenses/Jobs-Reset-GPL-3.0.txt`, and complete source is retained under `5. modpack source/custom mods/nbidal18-jobs-reset` in the organized release folder.

`nbidal18-safe-rejoin-1.0.0+1.21.1.jar` is a source-retained nbidal18 component. It makes leaving and rejoining while riding something survivable. Vanilla only saves a player's ride when exactly one player is aboard, so with two people in one aircraft neither ride is saved and whoever leaves first returns midair and falls; relaxing that condition is not the fix, because vanilla stores the vehicle as NBT and recreates it on load, which would duplicate the machine and its cargo. This stores the ridden entity's UUID instead and remounts only if it still exists, falling back to setting the player on solid ground. It also makes a moving or airborne vehicle count as activity, because no movement packet resets vanilla's idle timer and flying therefore reads as idling for pilot and passengers alike. Its MIT license is retained at `licenses/Safe-Rejoin-MIT.txt`, and complete source is retained under `5. modpack source/custom mods/nbidal18-safe-rejoin` in the organized release folder.

`nbidal18-discovery-plaques-1.0.0+1.21.1.jar` is a source-retained nbidal18 client component. It moves Field Guide's discovery notification onto an Advancement Plaques plaque, so a discovery appears where this pack's skill levels, job ranks and progression milestones already appear rather than as a vanilla toast in the opposite corner. It ships no third-party code or assets. It reads three public methods of Field Guide by reflection - the last-unlock stamp, the last-unlocked entry, and the entry-name helper - because Field Guide is built with a newer Fabric Loom than this pack's projects use and Loom will not remap such a jar onto a compile classpath; the source says so at the point where it matters. It does **not** call Advancement Plaques at all: it hands the vanilla toast system an ordinary `AdvancementToast`, which Advancement Plaques intercepts by its own design, so removing that mod leaves a plain vanilla toast rather than an error. The advancement it builds is synthetic - never registered, never sent to a server, never saved - and exists only to be drawn. Field Guide's own toast is turned off in `config/fieldguide-client.json` so the two do not both fire; the unlock sound is a separate setting and is left on. Its MIT license is retained at `licenses/Discovery-Plaques-MIT.txt`, and complete source is retained under `5. modpack source/custom mods/nbidal18-discovery-plaques` in the organized release folder.

`nbidal18-wiki-1.5.0+1.21.1.jar` is a source-retained nbidal18 client component. It is the pack's own in-game wiki: a keybind, or a button on the pause menu, opens a book of sections explaining how this modpack works, with no item to carry and nothing gated. It draws Field Guide's book, ruled-page, tab and page-turn textures, referenced by resource location and loaded from that mod's own JAR at runtime. The icons on the tabs are this pack's own artwork, drawn for it in Field Guide's palette rather than taken from it; they were item renders until 1.5.0. **No third-party asset is copied into or redistributed with this component**; it declares Field Guide as a hard dependency because without it there would be nothing to draw, and a resource pack that restyles Field Guide restyles the wiki with it. Note that Field Guide's assets are All Rights Reserved, and its book art is itself adapted from another mod under a permission granted to that project, so the permission covering this reuse is the pack owner's to hold rather than anything this notice conveys. Earlier versions rebuilt the look from vanilla's book and advancement sprites instead; that was replaced on the owner's call because it did not match. Its MIT license is retained at `licenses/Wiki-MIT.txt`, and complete source, including every line of the wiki's text, is retained under `5. modpack source/custom mods/nbidal18-wiki` in the organized release folder.
...
## Skills, jobs and gear tiers

**Jobs+ was removed in v4.3.0** and replaced by LevelZ and JobsAddon. Arc Lib 9.0.0 (`H3eKhxi7` / `dophvkyy`), UI Lib 9.0.0 (`AOEDs9Al` / `54ffhTnM`), YAML Config 9.0.0 (`L1FFB418` / `ZkxIJR6Z`) and Item Restrictions 9.0.0 (`rU60qFq2` / `2UCGjLwS`) **remain bundled**: Item Restrictions is still in the pack in its own right and Arc is its dependency. Projects: <https://modrinth.com/mod/arc>, <https://modrinth.com/mod/ui-lib>, <https://modrinth.com/mod/yaml-config> and <https://modrinth.com/mod/item-restrictions>, all declaring Apache-2.0.

The byte-exact `levelz-2.0.10.jar` (`EFtixeiF` / `xOVw5rrn`) and `jobsaddon-1.2.5.jar` (`whv2X0ei` / `eXKSBIB3`) are bundled on both sides, together with their shared `libz-1.1.0.jar` (`yUBXc3AH` / `tvtFvW21`) dependency. Projects: <https://modrinth.com/mod/levelz>, <https://modrinth.com/mod/jobsaddon> and <https://modrinth.com/mod/libz>, with source at <https://github.com/Globox1997/LevelZ>. LevelZ and JobsAddon declare GPL-3.0; LibZ declares MIT. The pack changes configuration and data only: content restrictions are switched off entirely, and the shipped skill table is replaced by the pack's own under `datapacks/nbidal18_progression`.

The byte-exact `tiered-1.3.7.jar` (`z6sMEexp` / `4wft2iIh`) is bundled on both sides. Project: <https://modrinth.com/mod/tieredz>, source at <https://github.com/Globox1997/tiered>, declaring MIT. It shares the LibZ dependency above and needs nothing else. The pack disables its anvil reforging and its rarity tooltip in configuration, and republishes all 144 of its item modifiers with neutral styling under `datapacks/nbidal18_gear_tiers`, which also adds Epic Knights' shields to the convention tag its shield modifiers key on.

## Field Guide, Item Descriptions and Mod Descriptions

The byte-exact `fieldguide-fabric-1.21.1-1.15.2.jar` (`72RRCWM6` / `m5phd0ws`) is bundled on both sides, and the byte-exact `item_descriptions-2.8.0+1.21.1-fabric.jar` (`UaizcMKP` / `qgkp5Cw5`) on the client only, being a client mod. The byte-exact `mod-descriptions-v12.0.zip` resource pack (`DkF7ssGR` / `AF2NmxaJ`) is bundled and enabled. Projects: <https://modrinth.com/mod/field-guide>, <https://modrinth.com/mod/item-descriptions> and <https://modrinth.com/resourcepack/mod-descriptions>, with Field Guide's source at <https://github.com/evanbones/Field-Guide>.

**Field Guide's licence is split and is not plain MIT.** Its source code is MIT; its assets are **All Rights Reserved** by their creators. The JAR ships unmodified, as every other bundled mod does, so nothing here depends on the distinction — but recording it as MIT alone would be wrong. Item Descriptions and Mod Descriptions declare MIT.

Field Guide is configured for discovery: a spyglass is required to scan, nothing is unlocked passively, undiscovered entries stay hidden and out of search, and the field guide item is disabled entirely so the guide opens from its keybind alone.
