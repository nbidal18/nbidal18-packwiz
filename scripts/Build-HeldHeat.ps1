...
foreach ($key in 'archives_base_name', 'mod_version') {
    if (-not $props.ContainsKey($key)) { throw "gradle.properties is missing '$key' for the held heat mod." }
}
...
try {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "The held heat mod build failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        Pop-Location
...
if (-not (Test-Path -LiteralPath $artifact -PathType Leaf)) {
    throw "The held heat mod artifact is missing: $artifact"
}
...
foreach ($destination in $destinations) {
    ...
    foreach ($stale in Get-ChildItem -LiteralPath $destination -File -Filter "$baseName-*.jar") {
        ...
        if (-not $resolvedStale.StartsWith($destination + '\', [StringComparison]::OrdinalIgnoreCase)) {
            throw "held heat mod cleanup escaped the mods directory: $resolvedStale"
        }
    ...
    if ($installed.Count -ne 1 -or $installed[0].Name -ne $artifactName) {
        throw "The canonical pack does not contain exactly the expected held heat mod artifact in $destination."
    }
    Write-Host "Built and installed held heat mod: $(Join-Path $destination $artifactName)"
}
