...
foreach ($key in 'archives_base_name', 'mod_version') {
    if (-not $props.ContainsKey($key)) { throw "gradle.properties is missing '$key' for the JEI coverage bridge." }
}
...
# Client-only mod: JEI itself is a client concern, so this bridge is never installed on the server.
...
try {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "The JEI coverage bridge build failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        Pop-Location
...
if (-not (Test-Path -LiteralPath $artifact -PathType Leaf)) {
    throw "The JEI coverage bridge artifact is missing: $artifact"
}
...
foreach ($destination in $destinations) {
    ...
    foreach ($stale in Get-ChildItem -LiteralPath $destination -File -Filter "$baseName-*.jar") {
        ...
        if (-not $resolvedStale.StartsWith($destination + '\', [StringComparison]::OrdinalIgnoreCase)) {
            throw "JEI coverage bridge cleanup escaped the mods directory: $resolvedStale"
        }
    ...
    if ($installed.Count -ne 1 -or $installed[0].Name -ne $artifactName) {
        throw "The canonical pack does not contain exactly the expected JEI coverage bridge artifact in $destination."
    }
    Write-Host "Built and installed JEI coverage bridge: $(Join-Path $destination $artifactName)"
}
