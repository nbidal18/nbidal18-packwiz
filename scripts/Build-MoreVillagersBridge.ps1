...
foreach ($key in 'archives_base_name', 'mod_version') {
    if (-not $props.ContainsKey($key)) { throw "gradle.properties is missing '$key' for the More Villagers bridge." }
}
...
# Installed on both sides: the coin conversion must work in dedicated and integrated worlds alike.
$destinations = @(
...
try {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "The More Villagers bridge build failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        Pop-Location
...
if (-not (Test-Path -LiteralPath $artifact -PathType Leaf)) {
    throw "The More Villagers bridge artifact is missing: $artifact"
}
...
foreach ($destination in $destinations) {
    ...
    foreach ($stale in Get-ChildItem -LiteralPath $destination -File -Filter "$baseName-*.jar") {
        ...
        if (-not $resolvedStale.StartsWith($destination + '\', [StringComparison]::OrdinalIgnoreCase)) {
            throw "More Villagers bridge cleanup escaped the mods directory: $resolvedStale"
        }
    ...
    if ($installed.Count -ne 1 -or $installed[0].Name -ne $artifactName) {
        throw "The canonical pack does not contain exactly the expected More Villagers bridge artifact in $destination."
    }
    Write-Host "Built and installed More Villagers bridge: $(Join-Path $destination $artifactName)"
}
