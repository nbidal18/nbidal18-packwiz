...
foreach ($key in 'archives_base_name', 'mod_version') {
    if (-not $props.ContainsKey($key)) { throw "gradle.properties is missing '$key' for the safe rejoin mod." }
}
...
# Installed on both sides: the rules must hold in dedicated and integrated worlds alike.
$destinations = @(
...
try {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "The safe rejoin mod build failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        Pop-Location
...
if (-not (Test-Path -LiteralPath $artifact -PathType Leaf)) {
    throw "The safe rejoin mod artifact is missing: $artifact"
}
...
foreach ($destination in $destinations) {
    ...
    foreach ($stale in Get-ChildItem -LiteralPath $destination -File -Filter "$baseName-*.jar") {
        ...
        if (-not $resolvedStale.StartsWith($destination + '\', [StringComparison]::OrdinalIgnoreCase)) {
            throw "safe rejoin mod cleanup escaped the mods directory: $resolvedStale"
        }
    ...
    if ($installed.Count -ne 1 -or $installed[0].Name -ne $artifactName) {
        throw "The canonical pack does not contain exactly the expected safe rejoin mod artifact in $destination."
    }
    Write-Host "Built and installed safe rejoin mod: $(Join-Path $destination $artifactName)"
}
