...
- The published setup archive is also `nbidal18-client.zip`, with no version in the name, so the download link stays the same across releases.
- The HUD auto-hides in three tiers. **Never shown:** the armour bar, the experience bar and the mount jump bar. **Shown while changing, on its own:** the air bar underwater. **The hotbar group** — hotbar slots, hearts, food, vehicle damage, the temperature overlay, saddle health and the experience level — has no trigger of its own; any member's trigger reveals all of it, and breaking or placing a block is the hotbar's trigger. Auto HUD's own screen-wide reveal ignores per-component policy, so it is redirected to the group rather than the whole HUD. Temperature reveals only when it moves by half a heart, because revealing while merely cold held the group open indefinitely.
...
- Every published config file carries an explicit classification in `scripts/config-classification.json`, and the build refuses to run while any is unclassified. `gameplay` files are managed before launch and hash-enforced during multiplayer. `support` files - libraries, performance, rendering, UI, input and diagnostics - are not in-game content, so they are restored by the pre-launch updater but never enforced at login. `player` files (Auto HUD, personal Voice Chat state, Iris shader selection, mod keybinds, controller state, base Sodium graphics preferences) are published once and preserved forever. Sodium's generated hardware fingerprint is never published.
...
- Better Compatibility Checker 21.1.8 gives the Packwiz client identity `v4.2.0-packwiz`. The server requires that exact BCC identity and `require-helper=true`, so older clients and clients without a valid helper attestation are rejected.
- The client verifies managed artifacts from the last successful manifest during multiplayer login and monitors them while connected. Config files are enforced according to their classification. A confirmed protected-artifact change disconnects the client and remains sticky until Minecraft is restarted.
...
- Unmanaged files that appear under `config/` are tolerated rather than refused. Config libraries write their own files while Minecraft starts, which the updater cannot undo: it deletes them, the game recreates them during mod init, and the player is locked out on every launch with no way to clear it. The loadable roots - `mods`, `datapacks`, `resourcepacks` and `shaderpacks` - stay strictly exact-match, and managed config files are still hash-checked, so nothing about tamper detection is relaxed.
- Every gameplay config stays hash-enforced at login, including the 54 rules covering files their own mod rewrites during startup. Those rewrites are byte-identical — the mod serialises back exactly what was published — so enforcing them costs nothing and keeps the check meaningful for a client that skips the pre-launch updater. Such files are marked `modWritesAtRuntime`, which is a watch list rather than an exemption: it records that the file must be published in the exact form the mod writes back. `scripts/Test-ConfigStability.ps1` measures this against a played instance and fails the release if any enforced config no longer round-trips, so a mod update that changes its serialisation is caught before players meet it as a login refusal.
