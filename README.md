...
- The v4.1.3 client verifies immutable artifacts from the last successful manifest during multiplayer login and monitors them while connected. Mod-owned `config` files are repaired before launch but allowed to change during play; explicit security-critical property rules remain enforced. A confirmed protected-artifact change disconnects the client and remains sticky until Minecraft is restarted.
