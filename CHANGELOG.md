...
A maintenance release. Nothing changes in game — two small files are re-downloaded and
the server is stricter about which version may connect.

| Date | Commit | Digest | Files | Mods | Change |
| --- | --- | --- | --- | --- | --- |
| 2026-08-20 | `7e65b41` | `ae4056dd367ba5b3` | 862 | 254 | **Only the current release can join the server now.** The server used to keep a list of older releases it would also accept, so a client a version or two behind could still log in. That list existed to cover the minute or two between a release going live on the server and going live on the download channel — and it was not worth what it cost. It grew every time the pack was built rather than every time it was published, so it filled with versions no player ever had; it once reached sixty entries and overflowed the size limit the login check reads, at which point the check treated its own policy as missing and the server stopped enforcing anything at all. Worse, keeping an old version on that list is a promise that it still works with the current server, which nothing was checking and which stops being true the moment a release adds a mod. So there is no list any more: one current version, and a client that is behind is told to close and reopen its game — which is exactly what runs the updater. Nothing about this is visible in game. Maintainer-facing: the publish order is reversed to match, pushing to the channel first and taking the server down only once the channel is confirmed live, which keeps the lockout window inside the restart instead of stacked on top of it. |
...
Backups under `Z:\.nbidal18-deploy-backups\2026-08-20-v4.4.1\`, each verified by hash on an independent second read.

---

## v4.4.0

Superseded by v4.4.1. Live digest `a02d4d60cb6d45ad…`. 862 managed files, 254 mods.
...
**If you have a hat, it is worth wearing now.** Sun protection was doing nothing until this
release, so a desert loadout that felt pointless was not your imagination.

| Date | Commit | Digest | Files | Mods | Change |
| --- | --- | --- | --- | --- | --- |
| 2026-08-20 | `d5679dc` | `0393a4a97b9993a2` | 857 | 253 | **Sun hats finally do something.** Every hat this pack added to Scorchful's sun-protection tag — the turtle helmet, every Trinkets hat slot, the Artifacts head and face slots — has been purely decorative since the day it was added. Scorchful hardcodes that protection onto its own sun hat; the tag is read in exactly one place, and its only callers are a HUD overlay and a renderer. So the wiki has been telling you a hat beats a desert while the hats did nothing. They now grant the same 0.25 heat resistance Scorchful's own hat does. It is applied by scanning what you are wearing rather than through the item, because a trinket contributes no vanilla attribute modifiers and most of that tag is trinket hats — the obvious fix would have worked for the turtle helmet and silently done nothing for the rest. Best hat counts, not the sum. **The second ring slot is gone — properly this time.** v4.3.3 said it removed the dead glove slot and did not: Trinkets reads its slot files with `listResourceStacks`, so our override was merged into Artifacts' list rather than replacing it, and the slot survived. It needed a `replace` flag. **The experience level is vanilla green again.** v4.3.3's fix overshot: Auto HUD folds an alpha into that colour to fade the HUD, so the four black outline passes stopped being pure black, failed our test for it, and got painted green too — a green number inside a green outline. It now changes exactly one value and leaves the fade alone. **The wiki is Field Guide's book.** It was a vanilla book pretending; now it draws Field Guide's own cover, ruled pages, tabs and page arrows, so the pack's two books are one object. It is a two-page spread, which nearly doubles what fits, and it has a search box: type to jump to a match, Enter for the next. No pixels are copied — the textures are drawn from Field Guide's own files at runtime, so a resource pack that restyles one restyles both. Maintainer-facing: wiki 1.4.0, temperature 2.1.0, client-tweaks 1.7.0. |

### Server deployment

Required a stopped server: the integrity helper carries the pack version, two mods changed, and
the trinket slot declaration is read at datapack load. A status ping refused the connection
immediately before the writes.

Backups under `Z:\.nbidal18-deploy-backups\2026-08-20-v4.3.4\`, each file verified by hash on an independent second read.

**The server was restarted as soon as the overlay was verified**, rather than being held down
through the changelog, commit, push and Pages deployment. Only the file overlay needs the server
stopped; everything after it is local or on GitHub, and the previous digest stays accepted
throughout, so nobody is locked out during the gap. That turns a twenty-minute outage into a
two-minute one.
...
Superseded by v4.3.4. Live digest `b631b45dc5381586…`. 857 managed files, 253 mods.
